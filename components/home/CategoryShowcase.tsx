...
export default function CategoryShowcase() {
  return (
    <section className="w-full bg-white py-1">
      <div className="w-full grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-1 sm:gap-1.5 px-1 sm:px-1.5">
        {CATEGORIES.map((cat) => (
          <Link
            key={cat.title}
            href={cat.href}
            className="group relative h-[440px] sm:h-[480px] lg:h-[540px] w-full overflow-hidden bg-neutral-900 flex items-end block"
          >
            ...
            <div className="relative z-10 p-6 sm:p-7 text-left w-full">
              {/* Category Title */}
              <h3
                style={{ fontFamily: "var(--font-cormorant), var(--font-playfair), serif" }}
                className="text-2xl sm:text-[26px] font-normal text-white tracking-[0.015em] leading-tight drop-shadow-md"
              >
                {cat.title}
              </h3>
